package dao.webtoon;

import java.util.Date;

public class WebtoonDTO {
	private String webtoon_idx;
	private String webtoon_title;
	private String webtoon_content;
	private String webtoon_summary;
	private String webtoon_author;
	private Date on_date;
	private String thum;
	private Date in_date;
	private String in_admin;
	private Date up_date;
	private String up_admin;
	private String use_yn;
	private String img;
	private String img_ori;
	private int dp_cnt;
	
	
	public String getWebtoon_idx() {
		return webtoon_idx;
	}
	public void setWebtoon_idx(String webtoon_idx) {
		this.webtoon_idx = webtoon_idx;
	}
	public String getWebtoon_title() {
		return webtoon_title;
	}
	public void setWebtoon_title(String webtoon_title) {
		this.webtoon_title = webtoon_title;
	}
	public String getWebtoon_content() {
		return webtoon_content;
	}
	public void setWebtoon_content(String webtoon_content) {
		this.webtoon_content = webtoon_content;
	}
	public String getWebtoon_summary() {
		return webtoon_summary;
	}
	public void setWebtoon_summary(String webtoon_summary) {
		this.webtoon_summary = webtoon_summary;
	}
	public String getWebtoon_author() {
		return webtoon_author;
	}
	public void setWebtoon_author(String webtoon_author) {
		this.webtoon_author = webtoon_author;
	}
	public Date getOn_date() {
		return on_date;
	}
	public void setOn_date(Date on_date) {
		this.on_date = on_date;
	}
	public String getThum() {
		return thum;
	}
	public void setThum(String thum) {
		this.thum = thum;
	}
	public Date getIn_date() {
		return in_date;
	}
	public void setIn_date(Date in_date) {
		this.in_date = in_date;
	}
	public String getIn_admin() {
		return in_admin;
	}
	public void setIn_admin(String in_admin) {
		this.in_admin = in_admin;
	}
	public Date getUp_date() {
		return up_date;
	}
	public void setUp_date(Date up_date) {
		this.up_date = up_date;
	}
	public String getUp_admin() {
		return up_admin;
	}
	public void setUp_admin(String up_admin) {
		this.up_admin = up_admin;
	}
	public String getUse_yn() {
		return use_yn;
	}
	public void setUse_yn(String use_yn) {
		this.use_yn = use_yn;
	}
	public String getImg() {
		return img;
	}
	public void setImg(String img) {
		this.img = img;
	}
	public String getImg_ori() {
		return img_ori;
	}
	public void setImg_ori(String img_ori) {
		this.img_ori = img_ori;
	}
	public int getDp_cnt() {
		return dp_cnt;
	}
	public void setDp_cnt(int dp_cnt) {
		this.dp_cnt = dp_cnt;
	}
}
